...
<body>
    <table style="width: 100%; border-collapse: collapse;" border="1">
        ...
        <tr>
            ...
            <td colspan="3" style="text-align: right;">{{ $eligibility->insurance_name ?? '' }}</td>
        </tr>
        <tr>
            <td style="background-color: {{ $eligibility->network_status ? '#25fd3c' : '#ff524f' }}">In Network / Out of Network</td>
            <td colspan="3" style="text-align: right; background-color: {{ $eligibility->network_status ? '#25fd3c' : '#ff524f' }}">{{ $eligibility->network_status ? 'In Network' : 'Out of Network' }}</td>
        </tr>
        <tr>
            <td>Member ID</td>
            <td colspan="3" style="text-align: right;">{{ $eligibility->member_id ?? '' }}</td>
        </tr>
        <tr>
            <td>Group Name</td>
            <td colspan="3" style="text-align: right;">{{ $eligibility->group_name ?? '' }}</td>
        </tr>
        <tr>
            <td>Group Number</td>
            <td colspan="3" style="text-align: right;">{{ $eligibility->group_number ?? '' }}</td>
        </tr>
        <tr>
            <td>Effective Date</td>
            <td colspan="3" style="text-align: right;">{{ viewDate($eligibility->effective_date) ?? '' }}</td>
        </tr>
        <tr>
            <td>End Date</td>
            <td colspan="3" style="text-align: right;">{{ viewDate($eligibility->end_date) ?? '' }}</td>
        </tr>
        <tr>
            <td>Claims Filling Limit</td>
            <td colspan="3" style="text-align: right;">{{ $eligibility->claims_filing_limit ?? '' }}</td>
        </tr>
        <tr>
            <td>Life Time</td>
            <td colspan="3" style="text-align: right;">{{ $eligibility->life_time ?? '' }}</td>
        </tr>
        
        <tr>
            <td>Waiting Period</td>
            <td style="text-align: right;">{{ $eligibility->waiting_period ?? '' }}</td>
            <td colspan="2" style="text-align: right;">{{ $eligibility->waiting_period_remarks ?? '' }}</td>
        </tr>
        <tr>
            <td>Missing Tooth Clause</td>
            <td style="text-align: right;">{{ $eligibility->missing_tooth_clause ?? '' }}</td>
            <td colspan="2" style="text-align: right;">{{ $eligibility->missing_tooth_clause_remarks ?? '' }}</td>
        </tr>

        <tr>
            <td style="background-color: #fff1cf">Ortho Maximum</td>
            <td colspan="3" style="text-align: right; background-color: #fff1cf">
        ...
        <tr>
            ...
            <td colspan="3" style="text-align: right; background-color: #fff1cf">{{ $eligibility->ortho_age_limit ?? '' }}</td>
        </tr>
        <tr>
        ...
        <tr>
            <td>Plan Year</td>
            <td colspan="3" style="text-align: right;">{{ $eligibility->plan_year ?? '' }}</td>
        </tr>
        <tr class="highlight">
            <td></td>
        ...
        <tr>
            ...
            <td colspan="3" style="text-align: right;">{{ $eligibility->deductible_applies_to ?? '' }}</td>
        </tr>
        <tr>
            <td>Preventive Waived</td>
            <td colspan="3" style="text-align: right;">{{ $eligibility->preventive_waived ?? '' }}</td>
        </tr>
        ...
        <tr>
            <td colspan="4" style="background-color: #21fffe; font-weight: bold; text-align: center;">Frequency and History</td>
        </tr>
        <tr>
            <td style="font-weight: bold; text-align: center;"></td>
            <td colspan="2" style="background-color: #FFFF00; font-weight: bold; text-align: center;">Frequency</td>
            <td style="background-color: #FFFF00; font-weight: bold; text-align: center;">History</td>
        </tr>
        @foreach(getEligibilityFormFieldsArray()['examDataArray'] as $key => $value)
            <tr>
                <td>{{ $value }}</td>                
                <td colspan="2" style="text-align: left;">{{ $examData[$key]['frequency'] ?? '' }}</td>
                <td style="text-align: right;">{{ $examData[$key]['history'] ?? '' }}</td>
            </tr>
        @endforeach
        ...
        @foreach(getEligibilityFormFieldsArray()['shareHistoryArray'] as $field => $value)
            <tr>
                <td>{{ $value }}</td>                
                <td colspan="3" style="text-align: center;">{{ $shareHistoryData[$field] ?? '' }}</td>
            </tr>
        @endforeach
